...
class Messages {
    ...
    getMessage(field, error, lang) {
        ...
        const template = objectPath.get(this.customMessages.custom, key)
            ...
            || key;
        return Object.keys(error.params).reduce((message, param) => message.replace(`:${param}`, error.params[param]), template).replace(":attribute", Validator_1.Validator.attributeFormatter(field));
    }
}
